...
function renderProducts() {
  ...
  document.getElementById("product-grid").innerHTML = visible.map(product => `<article class="product-card" data-product="${product.id}" tabindex="0"><div class="product-art"><span>${product.type === "image" ? "âœ¦" : product.type === "number" ? "615" : "hello"}</span></div><div class="product-meta"><h3>${product.name}</h3><p>${product.places}</p><strong class="price">from ${money(product.price)}</strong></div></article>`).join("");
...
>>>>>>> 17055d4 (Initial website commit)
